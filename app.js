...
function mostrarAba(id) { document.querySelectorAll('.aba').forEach(e => e.classList.remove('ativa')); document.querySelectorAll('.btn-nav').forEach(e => e.classList.remove('active')); document.getElementById(id).classList.add('ativa'); const btns = document.querySelectorAll('.btn-nav'); btns.forEach(btn => { if(btn.getAttribute('onclick') === `mostrarAba('${id}')`) btn.classList.add('active'); }); if(id === 'aba-relatorio') carregarRelatorioFiltrado(); if(id === 'aba-clientes') carregarClientesBase(); }
...
async function carregarResumoHome() { let total = 0, pendente = 0; const div = document.getElementById('listaRecentes'); div.innerHTML = ""; const snap = await db.collection("viagens").orderBy("timestamp", "desc").limit(5).get(); snap.forEach(doc => div.innerHTML += montarItemHTML(doc.id, doc.data(), false)); const full = await db.collection("viagens").get(); full.forEach(doc => { const p = doc.data(); if(p.status === 'Pago' || !p.status) total += p.valor; if(p.status === 'Pendente') pendente += p.valor; }); document.getElementById('totalCaixa').innerText = total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }); document.getElementById('totalPendentes').innerText = pendente.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }); }
